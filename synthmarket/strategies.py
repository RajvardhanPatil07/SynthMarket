...
TEMPLATE_DEFINITIONS: dict[str, dict[str, Any]] = {
    "buy_hold": {"label": "Buy and Hold", "description": "Stay fully invested after the first bar.", "parameters": {}},
    "sma_crossover": {
        "label": "SMA Crossover",
        "description": "Go long when the short simple moving average is above the long average.",
        "parameters": {
            "short_window": {"label": "Short SMA", "type": "int", "default": 20, "min": 2, "max": 252},
            "long_window": {"label": "Long SMA", "type": "int", "default": 50, "min": 3, "max": 512},
        },
    },
    "ema_crossover": {
        "label": "EMA Crossover",
        ...
        "parameters": {
            "short_window": {"label": "Short EMA", "type": "int", "default": 12, "min": 2, "max": 252},
            "long_window": {"label": "Long EMA", "type": "int", "default": 26, "min": 3, "max": 512},
        },
    },
    "rsi_mean_reversion": {
        ...
        "parameters": {
            "rsi_window": {"label": "RSI Window", "type": "int", "default": 14, "min": 2, "max": 252},
            "lower": {"label": "Entry RSI", "type": "float", "default": 30.0, "min": 1.0, "max": 99.0},
            "exit": {"label": "Exit RSI", "type": "float", "default": 50.0, "min": 1.0, "max": 99.0},
        },
    },
    "bollinger_mean_reversion": {
        ...
        "parameters": {
            "window": {"label": "Window", "type": "int", "default": 20, "min": 2, "max": 252},
            "entry_z": {"label": "Entry Z", "type": "float", "default": 2.0, "min": 0.1, "max": 6.0},
            "exit_z": {"label": "Exit Z", "type": "float", "default": 0.0, "min": -3.0, "max": 3.0},
        },
    },
    "donchian_breakout": {
        ...
        "parameters": {
            "lookback": {"label": "Entry Lookback", "type": "int", "default": 55, "min": 2, "max": 512},
            "exit_lookback": {"label": "Exit Lookback", "type": "int", "default": 20, "min": 2, "max": 512},
        },
    },
}
...
def list_strategy_templates() -> list[dict[str, Any]]:
    ...
    return [
        {"id": template_id, "label": definition["label"], "description": definition["description"], "parameters": definition["parameters"]}
        for template_id, definition in TEMPLATE_DEFINITIONS.items()
    ]


def make_strategy_spec(template: str = "sma_crossover", parameters: Optional[dict[str, Any]] = None, name: Optional[str] = None, description: str = "", advanced_code: str = "") -> StrategySpec:
    ...
    defaults = {key: value["default"] for key, value in TEMPLATE_DEFINITIONS[template]["parameters"].items() if "default" in value}
    return StrategySpec(name=name or TEMPLATE_DEFINITIONS[template]["label"], template=template, parameters={**defaults, **(parameters or {})}, description=description, advanced_code=advanced_code)
...
def strategy_from_payload(payload: Optional[dict[str, Any]]) -> StrategySpec:
    ...
    if "template" in payload:
        return make_strategy_spec(str(payload.get("template", "sma_crossover")), dict(payload.get("parameters") or {}), str(payload.get("name") or ""), str(payload.get("description") or ""), str(payload.get("advanced_code") or ""))
...
def validate_strategy_parameters(template: str, parameters: dict[str, Any]) -> None:
    ...
    for name, spec in definition["parameters"].items():
        ...
        parsed = _coerce_int(value, name, spec["min"], spec["max"]) if spec["type"] == "int" else _coerce_float(value, name, spec["min"], spec["max"])
...
def generate_strategy_signal(ohlcv: pd.DataFrame, strategy: StrategySpec) -> pd.Series:
    ...
    if strategy.template == "sma_crossover":
        short = close.rolling(int(params["short_window"]), min_periods=int(params["short_window"])).mean()
        long = close.rolling(int(params["long_window"]), min_periods=int(params["long_window"])).mean()
        return (short > long).astype(float).fillna(0.0)
    if strategy.template == "ema_crossover":
        short = close.ewm(span=int(params["short_window"]), adjust=False, min_periods=int(params["short_window"])).mean()
        long = close.ewm(span=int(params["long_window"]), adjust=False, min_periods=int(params["long_window"])).mean()
        return (short > long).astype(float).fillna(0.0)
    if strategy.template == "rsi_mean_reversion":
        rsi = _rsi(close, int(params["rsi_window"]))
        return _stateful_signal(rsi < float(params["lower"]), rsi > float(params["exit"]), frame.index)
    if strategy.template == "bollinger_mean_reversion":
        ...
        return _stateful_signal(zscore < -float(params["entry_z"]), zscore > float(params["exit_z"]), frame.index)
    if strategy.template == "donchian_breakout":
        entry_high = close.shift(1).rolling(int(params["lookback"]), min_periods=int(params["lookback"])).max()
        exit_low = close.shift(1).rolling(int(params["exit_lookback"]), min_periods=int(params["exit_lookback"])).min()
        return _stateful_signal(close > entry_high, close < exit_low, frame.index)
...
def strategy_summary(strategy: StrategySpec) -> str:
    ...
    params = ", ".join(f"{key}={value}" for key, value in _filled_parameters(strategy).items())
    return f"{strategy.name} ({label}{': ' + params if params else ''})"


def _filled_parameters(strategy: StrategySpec) -> dict[str, Any]:
    defaults = {key: spec.get("default") for key, spec in TEMPLATE_DEFINITIONS[strategy.template]["parameters"].items()}
...
def _stateful_signal(entry: pd.Series, exit_signal: pd.Series, index: pd.Index) -> pd.Series:
    position = 0.0
    values: list[float] = []
    for should_enter, should_exit in zip(entry.fillna(False).to_numpy(), exit_signal.fillna(False).to_numpy(), strict=False):
        if should_exit:
            position = 0.0
        elif should_enter:
            position = 1.0
