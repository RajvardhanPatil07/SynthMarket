...
class StrategySpec:
    ...
    V2 intentionally starts with named templates plus parameters. The no-code
    builder can save this safely as JSON, while a later Python editor can map to
    the same output contract after sandboxing is added.
    """
...
def make_strategy_spec(
    ...
    label = TEMPLATE_DEFINITIONS[template]["label"]
    return StrategySpec(
        name=name or label,
        template=template,
        parameters=merged,
...
def generate_strategy_signal(ohlcv: pd.DataFrame, strategy: StrategySpec) -> pd.Series:
    ...
    if strategy.template == "donchian_breakout":
        ...
        exit_low = close.shift(1).rolling(
            int(params["exit_lookback"]), min_periods=int(params["exit_lookback"])
        ).min()
...
def _filled_parameters(strategy: StrategySpec) -> dict[str, Any]:
    definition = TEMPLATE_DEFINITIONS[strategy.template]
    defaults = {key: spec.get("default") for key, spec in definition["parameters"].items()}
...
def _stateful_signal(entry: pd.Series, exit_signal: pd.Series, index: pd.Index) -> pd.Series:
    position = 0.0
    values: list[float] = []
    for should_enter, should_exit in zip(entry.fillna(False).to_numpy(), exit_signal.fillna(False).to_numpy()):
        if should_exit:
            position = 0.0
        elif should_enter:
            position = 1.0
